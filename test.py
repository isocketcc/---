...
class Application():
    ...
    #-----------------------------------------------------------统计--------------------------------------------
    def add_paper(self):
        if os.path.exists("resEnglish\\统计\\all.txt"):
            #print("文档统计完毕!")
            ...
            endNum = 499
            paperNum = begNum
            result_paper = "resEnglish\\统计\\all.txt"
            result_paper_fw = open(result_paper, 'w')
            while paperNum < endNum:
                source_paper = "resEnglish\\停用词\\res\\" + str(paperNum) + "_D.txt"
    ...
    def set_cluster(self,value):
        ...
        corpus = []
        # 读取预料 一行预料为一个文档
        for line in open('resEnglish\\统计\\all.txt', 'rb').readlines():
        ...
        '''resName = "resEnglish\\统计\\BHTfidf_Result.txt"
